...
class GaussianDiffusion:
    ...
    def p_mean_variance(  # decode #4
        ...
        B, C = x.size(0), x.size(-1)  # B: batch_size
        assert t.shape == (B,)
        # print(x.shape)
        ...
        def process_xstart(x):
            if denoised_fn is not None:
                # print(denoised_fn)
                x = denoised_fn(x, t)
            if clip_denoised:  # 假 sample_seq2seq line 33: decode_defaults = dict(split='valid', clamp_step=0, seed2=105, clip_denoised=False)
                return x.clamp(-1, 1)
            return x

        if self.predict_xstart:  # 真
            pred_xstart = process_xstart(model_output)  # rounding
        else:
    ...
    def p_sample_loop_progressive(  # decode #2
        self,
        model,
        shape,
        noise=None,
    ...
    def training_losses_seq2seq(self, model, x_start, t, vnet=None, is_meta_run=False, model_kwargs=None, noise=None):
        """
        Compute training losses for a single timestep.

        :param model: the model to evaluate loss on. 传入model本身或者meta model
        ...
        """
        x_start_fix = x_start[0]  # save the orignal x_0
        input_ids_x = model_kwargs.pop('input_id_y').to(t.device)  # y是完整的句子，不含[MASK]
        input_ids_y = model_kwargs.pop('input_id_x').to(t.device)
        input_ids_mask = model_kwargs.pop('input_mask').to(t.device)
        x_start_mean = model.model.module.get_embeds(input_ids_x)  # X_0
        
        std = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
                                   th.tensor([0]).to(x_start_mean.device),
                                   x_start_mean.shape)

        x_start = self._get_x_start(x_start_mean, std)  # Word embedding projection from {Emb(w)} to {x_0}, 加噪一步

        if noise is None:
            noise = th.randn_like(x_start)

        x_t = self.q_sample(x_start, t, noise=noise, mask=input_ids_mask) # reparametrization trick.  # todo: check mask broadcast

        get_logits = model.model.module.get_logits

        terms = {}

        target = x_start_mean  # todo: 增加元学习 weight
        model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)  # transformer 预测的X_start  # todo check model_kwargs
        assert model_output.shape == target.shape == x_start.shape
        terms["mse"] = mean_flat((target - model_output) ** 2)  # todo: 增加元学习 weight  {Tensor: (10,)}
        if vnet and is_meta_run == 0:
            cost_v = th.reshape(terms["mse"], (len(terms["mse"]), 1))  # reshape {Tensor: (10,1)}
            v_lambda = vnet(cost_v.data)  # {Tensor: (10,1)}
            terms["mse"] = th.squeeze(cost_v * v_lambda)  # {Tensor: ()}
        if vnet and not is_meta_run == 2:
            cost_v = th.reshape(terms["mse"], (len(terms["mse"]), 1))  # reshape
            with th.no_grad():
                v_lambda = vnet(cost_v.data)
            terms["mse"] = th.squeeze(cost_v * v_lambda)
        ...
        out_mean, _, _ = self.q_mean_variance(x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device))  # return A tuple (mean, variance, log_variance), all of x_start's shape.
        ...
        terms["loss"] = terms["mse"] + decoder_nll + tT_loss  # todo: 考虑损失权重
    ...
    def ddim_sample_loop(
        ...
        for sample in self.ddim_sample_loop_progressive(
            ...
            final.append(sample['sample'])
        return final  # 1个batch所有时间步的采样结果，按时间步倒排 e.g. [[bsz, 128, 128](t==1999), ...]
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            sample_x = th.randn(*shape, device=device)
        indices = list(range(self.num_timesteps))[::-1][::gap]  # 先按将时间步逆序[1999,...,0] 再以gap为间隔采样[1999, 1999-gap...]
        ...
        for i in indices:  # [1999, 1999-gap...]
            t = th.tensor([i] * shape[0], device=device)  # shape = (bsz, seq_len, emb_dim)  e.g. t = [1999] * bsz
            with th.no_grad():
...
class SpacedDiffusion(GaussianDiffusion):  # GaussianDiffusion的子类
    """

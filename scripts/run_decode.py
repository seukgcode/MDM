...
if __name__ == '__main__':
    ...
    for lst in glob.glob(args.model_dir):
        print(lst)
        checkpoints = sorted(glob.glob(f"{lst}/{args.pattern}*.pt"))[::-1]  # 所有ckpt按由新到旧排列，[::-1]为取逆序

        out_dir = 'generation_outputs'
        if not os.path.isdir(out_dir):
            os.mkdir(out_dir)

        for checkpoint_one in checkpoints:  # 对每个ckpt都进行一次生成
            ...
            print(COMMAND)
            # python -m torch.distributed.launch --nproc_per_node=1 --master_port=12356 --use_env
            # sample_seq2seq.py --model_path diffusion_models/diffugen_e2e_h128_lr0.0001_t2000_sqrt_lossaware_seed102_e2e20221204-13:27:37/ema_0.9999_022000.pt --step 2000 --batch_size 50 --seed2 123 --split test --out_dir generation_outputs --top_p -1
            os.system(COMMAND)
